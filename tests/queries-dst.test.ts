...
describe('the spring-forward gap', () => {
  it('pushes a nonexistent fixed time forward in real time', () => {
    ...
    assert.equal(
      matches(d, schedule, '2026-03-08T03:30:00-04:00'),
      true,
      '02:30 EST read with the pre-transition offset',
    );
    assert.deepEqual(
...
describe('the fall-back overlap', () => {
  it('counts an ambiguous fixed time as its first occurrence only', () => {
    ...
    assert.equal(
      matches(d, schedule, '2026-11-01T01:30:00-05:00'),
      false,
      'the second pass is not a scheduled point',
    );
    assert.deepEqual(
...
describe('the interval every across transitions', () => {
  it('keeps the wall-clock row intact over spring forward', () => {
    ...
    assert.deepEqual(
      strings(
        occurrencesIn(d, d.schedules[0]!, '2026-03-07T12:00:00-05:00', '2026-03-08T12:00:00-04:00'),
      ),
  ...
  it('pushes a row point that lands in the gap forward', () => {
    ...
    assert.deepEqual(
      strings(
        occurrencesIn(d, d.schedules[0]!, '2026-03-08T00:00:00-05:00', '2026-03-08T12:00:00-04:00'),
      ),
      ['2026-03-08T03:30:00-04:00'],
    );
  });
  ...
  it('reads a row point in the overlap as its first occurrence', () => {
    ...
    assert.deepEqual(
      strings(
        occurrencesIn(d, d.schedules[0]!, '2026-11-01T00:00:00-04:00', '2026-11-01T12:00:00-05:00'),
      ),
      ['2026-11-01T01:30:00-04:00'],
    );
    assert.equal(
...
describe('a zone that skips a whole day', () => {
  ...
  it('denotes the day that follows for an all-day occurrence', () => {
    ...
    const withName = doc(
      'Pacific/Apia',
      { days: ['moved'], allday: true },
      { date_sets: { moved: ['2011-12-30'] } },
    );
  ...
  it('collapses two calendar days whose starts resolve to the same instant', () => {
    ...
    assert.deepEqual(
      strings(
        occurrencesIn(d, d.schedules[0]!, '2011-12-28T00:00:00-11:00', '2012-01-02T00:00:00+14:00'),
      ),
      ['2011-12-31'],
      'December 2011 only',
    );
